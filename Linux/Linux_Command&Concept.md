...
* `|` : 2개의 명령어를 연결해주는 연결 통로
* `명령1 | 명령2` : 명령2의 인자로 명령1을 넘겨준다.
  * `ls | grep link` : ls의 목록중 link 문자열을 가지고 있는 파일명들을 보여준다.

#### 리다이렉션

* 모든 터미널 명령은 표준 입력 키보드, 표준 출력 모니터이다.
* 리다이렉션은 표준 입출력의 방향을 바꿔준다.
* 명령어 결과를 출력파일로 지정
  * `명령어 > 파일명` : 저장(기존내용 삭제)
    * `echo "a abc afgdfg" > sample.txt`
  * `명령어 >> 파일명` : 저장(기존내용에 추가)
...
* `grep` : 파일에서 특정 문자열을 검색
  * `find`는 파일을 검색한다는 것에서 차이가 있다.
  * 정규표현식을 사용할 수 있다.
    * `grep '[a-z]' sample.txt` : `[]`는 찾고싶은 문자열 넣을때 사용
    * `grep 'a{2}' sample.txt` : `{}`는 숫자가 들어가며, 등장해야하는 횟수를 지정할때 사용
...
* `systemctl start/stop/restart/status network` : 네트워크 설정을 변경한 후 시스템에 적용시키는 명령
...
* `ps` : 현재 실행중인 프로세스를 살펴볼 수있다.
  * `ps -ef` : 실행했던 모든 프로세스 내역을 볼 수 있다.
    * 부모 프로세스의 번호도 나온다.
* `Foreground Process` : 화면에 나타나서 사용자와 상호작용을 하는 프로세스
  * Firefox 등
  * `gedit /etc/passwd`
    * gedit이 실행되어 있는 동안 터미널은 사용이 불가능하다
    * gedit이 foreground process이며, 터미널이 background process가 된다.
    * `gedit /etc/passwd &`로 `&`를 추가하면 터미널도 foreground process가 된다.
      * 명령어를 실행했을 때 나온 `[1]`뒤의 번호가 프로세스 번호이다.
      * `kill 프로세스번호`를 하면 해당 프로세스를 종료한다.
      * 프로세스 번호는 `ps ef | grep 프로세스 탐색 문자열`을 진행하여 찾을 수 있다.
* `Background Process` : 화면에 나타나지 않고 뒤에서 실행되는 프로세스
  * 바이러스 백신, 서버 데몬 등
  * `jobs` 명령어를 사용하면 현재 어떤 background process가 실행중인지 알 수 있다.

#### 서비스

> 평상시에도 늘 가동하는 서버 프로세스

* 데몬, 데몬 서비스 : 항상 실행중인 서비스
* `systemctl list-unit-files` : 서비스 목록을 살펴볼 수 있는 명령어
* `systemctl start/stop/restart 서비스이름` : 서비스 실행 및 종료
