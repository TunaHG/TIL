...
* 언제 사용하는가?
  * 멀티 프로세스의 장점을 활용할 때
  * 다수의 코어를 동시에 사용한다는 것으로, 동시에 두 개 이상의 연산을 수행하고자 할 때
...
* Multi Thread + Multi Process
  * 각각의 프로세스에서 멀티스레드가 돌아간다.
  * Java의 Multi Thread가 이러한 형태이다.
  * 각 프로세스의 Thread들이 지속적으로 돌아가며 필요한 연산들을 수행한다.
    * 멀티프로세스이므로 동시에 여러가지 연산이 가능하다.
...
* 단일 프로세스는 하나의 연산만을 수행
* 중간에 Thread가 쉴 때가 있는지에 따라 다름
  * 쉬는게 없을 땐, Single Thread!
    * Multi Thread를 이용해봤자 결국 단일 프로세스에서는 하나의 연산만 수행된다.
    * 여러 개의 연산이 동시에 수행되지 않으므로 Multi Thread의 의미가 없다.
    * Multi Thread로 이용한다면, Context Switch로 인해 불필요한 자원이 소모된다.
  * 쉬는게 있을 땐, Multi Thread
    * 하나의 Thread가 쉬는동안 다른 Thread가 작동할 수 있어서 의미가 있다.
